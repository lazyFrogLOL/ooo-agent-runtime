...
export interface Config {
  ...
  maxParallelToolCalls?: number
  /**
   * M1b benchmark path: a static task DAG executed out-of-order on the first
   * turn of every configured agent. Validated by {@link DAG_CONFIG_SCHEMA} in
   * the constructor; the schemastery side stays `unknown` so the two validator
   * families never mix.
   */
  dag?: unknown
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  private prepare(
    ...
    // Captured for the generator below, whose `this` is not the service.
    const dag = this.config.dag
    ...
    try {
      unfollowOwner = ownerCtx.effect(function* () {
        machine = new ReactLoopAgent(loopCtx, id, options, session, dag)
