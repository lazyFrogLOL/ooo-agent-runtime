...
export class ReactLoopAgent implements Agent {
  ...
  constructor(
    ...
    /**
     * M1b benchmark path: when set, turn 1 executes this static DAG through
     * the out-of-order scheduler instead of the conversational step loop.
     */
    private readonly dag?: DagConfig,
  ) {
  ...
  private async turn(): Promise<boolean> {
    ...
    try {
      // M1b benchmark path: the first turn of a DAG-configured loop executes
      // the declared task graph out-of-order, then closes the turn without
